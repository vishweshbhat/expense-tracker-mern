...
const ExpenseTable = () => {
  ...
  useEffect(() => {
    ...
    if (markAsDone && markAsDone !== "Both") {
      if (markAsDone === "done") {
        filteredExpenses = filteredExpenses.filter((expense) => !!expense.done);
    ...
    <Table>
      ...
      <TableFooter>
        <TableRow>
          ...
          <TableCell className="text-right font-bold text-xl">₹{totalAmount.toLocaleString()}</TableCell>
        </TableRow>
      </TableFooter>
    </Table>
  );
